...
func run() error {
	...
	*/
	...
	if err != nil {
		return xerrors.Errorf("failed to spawn ephemeral node: %s", err)
	}
	...
	if err != nil {
		return xerrors.Errorf("could not get File: %s", err)
	}

	cidFile, err := ipfs.Unixfs().Add(ctx, someFile)
	if err != nil {
		return xerrors.Errorf("could not add File: %s", err)
	}
	...
	if err != nil {
		return xerrors.Errorf("could not add Directory: %s", err)
	}
	...
	outputBasePath := "./example-folder/out-dir/"

	// Remove old out directory from previous run if exists and recreate empty one
	if _, err := os.Stat(outputBasePath); !os.IsNotExist(err) {
		fmt.Printf("Previous old directory exits: %s ", outputBasePath)

		if err := os.RemoveAll(outputBasePath); err != nil {
			return xerrors.Errorf("could not get file with CID: %s", err)
		}
		fmt.Println("... removed")
	}
	if err := os.Mkdir(outputBasePath, 0777); err != nil {
		return xerrors.Errorf("could not create dir: %s", err)
	}
	...
	if err != nil {
		return xerrors.Errorf("could not write out the fetched CID: %s", err)
	}
...
func setupPlugins(externalPluginsPath string) error {
	...
	if err != nil {
		return xerrors.Errorf("error loading plugins: %s", err)
	}

	// Load preloaded and external plugins
	if err := plugins.Initialize(); err != nil {
		return xerrors.Errorf("error initializing plugins: %s", err)
	}
...
func createTempRepo(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", xerrors.Errorf("failed to get temp dir: %s", err)
	}
	...
	if err != nil {
		return "", xerrors.Errorf("failed to init ephemeral node: %s", err)
	}
...
//lint:ignore U1000 Not used but keep it intentionaly since it was in the original
...
func spawnEphemeral(ctx context.Context) (icore.CoreAPI, error) {
	...
	if err != nil {
		return nil, xerrors.Errorf("failed to create temp repo: %s", err)
	}
